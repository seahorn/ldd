...
(** QF_AUFBV logic signature definitions. *)
...
(***************************************************************)
class logic_QF_AUFBV = object (self)
  ...
  initializer
    ...
    theory_sorts <- List.rev_append theory_sorts [arr];
    theory_funs <- List.rev_append theory_funs [
      CFun(Sym "select",[arr;bv],bv,[o],[Gt(m,zero);Gt(o,zero);Eq(m,m)]);
      CFun(Sym "store",[arr;bv;bv],arr,[m;o],[Gt(m,zero);Gt(o,zero);Eq(m,m);Eq(o,p)])
    ]
end
