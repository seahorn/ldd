...
(** QF_LIA logic signature definitions. *)
...
(***************************************************************)
class logic_QF_LIA = object (self)
  inherit Logic_smt.logic_smt

  initializer
  theory_sorts <- List.rev_append theory_sorts [integer];
  numeral_sort <- integer,[];
  theory_funs <- List.rev_append theory_funs [
    Fun(Sym "~",[integer],integer);
    Fun(Sym "-",[integer],integer);
    Fun(Sym "-",[integer;integer],integer);
    Fun(Sym "*",[integer;integer],integer);
    Fun(Sym "<=",[integer;integer],boolean);
    Fun(Sym "<",[integer;integer],boolean);
    Fun(Sym ">=",[integer;integer],boolean);
    Fun(Sym ">",[integer;integer],boolean);
    ExFun(Sym "+",integer,integer)
    ]
end
