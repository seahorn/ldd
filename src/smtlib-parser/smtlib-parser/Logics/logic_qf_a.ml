...
(** QF_A logic signature definitions. *)
...
(***************************************************************)
class logic_QF_A = object (self)
  inherit Logic_smt.logic_smt

  initializer
  theory_sorts <- List.rev_append theory_sorts [index;elem;arr];
  numeral_sort <- integer,[];
  theory_funs <- List.rev_append theory_funs [
    Fun(Sym "select",[arr;index],elem);
    Fun(Sym "store",[arr;index;elem],arr)
    ]
end
