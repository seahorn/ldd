...
// with 117 it is SAT, with 116 it isn't
#define MAX 117
...
void my_write_problem_smt(short p)  /* write out problem */ 
{
  short i, j, m, is, js;
  FILE *f = NULL;
  char name[5];

  sprintf(name,"ta%02d.smt", p);                 /* file name construction */ 
  if(!(f = fopen(name,"w"))) {               /* open file for writing  */ 
    fprintf(stderr,"file %s error\n", name);
    return;
  }
  //fprintf(f,"%d %d\n", S[p].num_jobs, S[p].num_mach);   /* write header line */ 
  fprintf(f, "(benchmark jobshop\n:logic QF_RDL\n");
  for (i = 0; i < S[p].num_jobs * S[p].num_mach; i++) fprintf(f, ":extrafuns ((x%d Real))\n", i);
			  ...
			  if (M[i][j] == m) {
				  for (is = i + 1 ; is < S[p].num_jobs; is++) { // now looking for next occuurrence of m
					  for (js = 0 ; js < S[p].num_mach; js++) { // columns
						  if (M[is][js] == m)  {
							  //fprintf(f, "S_%d_%d + %d <= S_%d_%d | S_%d_%d + %d <= S_%d_%d\n", i,j,d[i][j], is, js, is, js, d[is][js], i, j);
							  fprintf(f, "(or (<= (- x%d x%d) (- %d)) (<= (- x%d x%d) (- %d)))\n", id(i,j),id(is,js), d[i][j],  id(is, js), id(i,j), d[is][js]);
							  break;
						  }
					  }
				  }
