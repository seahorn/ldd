...
struct problem S[] = {
  {         0,         0, 0, 0},
 {1166510396, 164000672, 2, 2},
//  {1166510396, 164000672, 4, 4},  // the solution to this one is apparently under 200. I.e. with MAC = 200 it should be sat. 
  {         0,         0, 0, 0}};
...
{   682761130,  262763021,  100,  20 },
                                      /* 5 jobs, 5 machines */
{   840612802,  398197754,   5,  5 }, 
{  1314640371,  386720536,   5,  5 },
{  1227221349,  316176388,   5,  5 },
{   342269428, 1806358582,   5,  5 },
{  1603221416, 1501949241,   5,  5 },
{  1357584978, 1734077082,   5,  5 },
{    44531661, 1374316395,   5,  5 },
{   302545136, 2092186050,   5,  5 },
{  1153780144, 1393392374,   5,  5 },
{    73896786, 1544979948,   5,  5 },
                                        /* 10 jobs  5 machines */ 
{   533484900,  317419073,   10,  5 },
{  1894307698, 1474268163,   10,  5 },
{   874340513,  509669280,   10,  5 },
{  1124986343, 1209573668,   10,  5 },
{  1463788335,  529048107,   10,  5 },
{  1056908795,   25321885,   10,  5 },
{   195672285, 1717580117,   10,  5 },
{   961965583, 1353003786,   10,  5 },
{  1610169733, 1734469503,   10,  5 },
{   532794656,  998486810,   10,  5 },
{           0,          0,    0,   0 }};
#endif /* VERIFY */
...
#define id(a,b) a * S[p].num_mach + b 

void my_write_problem(short p)  /* write out problem */ 
{
  short i, j, m, is, js;
  FILE *f = NULL;
  char name[5];

  sprintf(name,"ta%02d.txt", p);                 /* file name construction */ 
  if(!(f = fopen(name,"w"))) {               /* open file for writing  */ 
    fprintf(stderr,"file %s error\n", name);
    return;
  }
  //fprintf(f,"%d %d\n", S[p].num_jobs, S[p].num_mach);   /* write header line */ 
  fprintf(f,"variables: %d\n", S[p].num_jobs * S[p].num_mach);
  
  for(i = 0; i < S[p].num_jobs; ++i) {
    for(j = 0; j < S[p].num_mach; ++j) {
		//fprintf(f,"S_%d_%d >= 0 \n",i,j); // (5.108)
		fprintf(f,"M %d 0\n", id(i,j));
		if (j < S[p].num_mach - 1) { //fprintf(f,"S_%d_%d + %d <= S_%d_%d\n",i,j,d[i][j],i,j+1); // (5.109)		
		fprintf(f,"%d %d %d\n",id(i,j),id(i,j+1), -d[i][j]); }// (5.109)		
  ...
  for (m = 0; m < S[p].num_mach; m++) {  // every machine  // (5.110)
	  for (i = 0 ; i < S[p].num_jobs; i++) { // rows
		  for (j = 0 ; j < S[p].num_mach; j++) { // columns
			  if (M[i][j] == m) {
				  for (is = i + 1 ; is < S[p].num_jobs; is++) { // now looking for next occuurrence of m
					  for (js = 0 ; js < S[p].num_mach; js++) { // columns
						  if (M[is][js] == m)  {
							  //fprintf(f, "S_%d_%d + %d <= S_%d_%d | S_%d_%d + %d <= S_%d_%d\n", i,j,d[i][j], is, js, is, js, d[is][js], i, j);
							  fprintf(f, "%d %d %d  %d %d %d\n", id(i,j),id(is,js), -d[i][j],  id(is, js), id(i,j), -d[is][js]);
							  break;
						  }
					  }
				  }
  ...
  for (i = 0 ; i < S[p].num_jobs; i++) { // rows // (5.111)
	  for (j = 0 ; j < S[p].num_mach; j++) { // columns
		  //fprintf(f, "S_%d_%d + %d <= %d\n", i, j, d[i][j], MAX);
		  fprintf(f, "%d M (MAX - %d)\n", id(i, j), d[i][j]);
	  }
  }
  fclose(f);                                 /* close file */ 
